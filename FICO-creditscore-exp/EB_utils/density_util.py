...
import random
from EB_utils.data_util import data_fetch
...
def simulation_echelon_cdf(option):
    ...
    if 'uniform' == option:
        ...
        domain_axis_adv = np.linspace(0.4, 0.9, 1000)
        # shared
        ...
        pdf_axis_disadv = np.exp(-0.5 * np.square(
            (domain_axis - 0.2) / 0.4))
        pdf_axis_adv = np.exp(-0.5 * np.square(
            (domain_axis - 0.8) / 0.4))
...
def predefine_feature_generate(protected_feature, echelon):
    """Generate X from (A, H).
    ...
    X_child_H = 1.5 * echelon + np.random.uniform(-0.1, 0.1)
    X_child_H_and_A = echelon + 0.8 * \
        protected_feature + np.random.uniform(-0.1, 0.1)
    X_child_A = protected_feature + np.random.normal(0, 0.1)

    # output
    X_not_protected_descendent = np.array([X_child_H, ])
    X_protected_descendent = np.hstack((X_child_H_and_A, X_child_A))
    return X_not_protected_descendent, X_protected_descendent


def adult_race_feature_generate():
    """Generate X from (A, H) for Adult race data.

    Parametric model to estimate P(X | A, H).
...
def adult_sex_feature_generate():
    """Generate X from (A, H) for Adult sex data.
...
def creditscore_echelon_cdf():
    ...
    # re-name the column
    df_cdf = df_cdf.rename(
        columns={'Non- Hispanic white': 'White'})

    # re-order columns
    df_cdf = df_cdf.reindex(
        ['Score', 'Black', 'Asian', 'Hispanic', 'White'], axis=1)
